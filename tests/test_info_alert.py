...
from src.info_alert import InformativeAlert
...
def create_scenario_alert(
    ...
    alert = InformativeAlert(exp_date, 'NA', subject)
    alert.set_user(user)
    return user, subject, alert


class TestInformativeAlert(TestCase):
    ...
    def test_send_and_check_if_user_has_the_alert(self):
        ...
        self.assertEqual(user.unread_alerts[0]['priority'], 'LOW')
    ...
    def test_send_urgent_alert_and_right_indexing(self):
        ...
        alert2 = InformativeAlert(exp_date, 'second_alert', subject)
        alert2.format_alert()
        alert2.send(user, alert2.format)
        self.assertTrue(user.unread_alerts[1] is alert2.format)
